jQuery(document).ready(function($) {
    ...
    function limpiarCamposComunaDuplicados() {
        // Borra TODOS los #shipping_city excepto el primero
        $('#shipping_city').not(':last').remove();
        //$('#shipping_city').not(':first').remove();
        // Borra contenedores select2 duplicados dentro del field
        $('#shipping_city_field .select2-container').not(':last').remove()
        //$('#shipping_city_field .select2-container').not(':first').remove();
    }
    ...
    function initAutocomplete() {
        ...
        google.maps.event.addListener(autocomplete, 'place_changed', function() {
            ...
            setTimeout(function() {
                limpiarCamposComunaDuplicados(); // <-- LLAMA AQUÍ
            
                var $cityField = $('#shipping_city').first();
                console.log("Aqui estoy OTRA VEZ");
                if ($cityField.is('input[type="text"]') && comunaCode) {
                    ...
                    if (regionComunas) {
                        ...
                        limpiarCamposComunaDuplicados();
            
                        if (typeof $.fn.select2 === 'function') {
                            $newCitySelect.select2();
                        }
            
                        // Llama de nuevo tras el reemplazo para limpiar si el replaceWith generó duplicados select2
                        limpiarCamposComunaDuplicados();
            ...
            $(document.body).trigger('update_checkout');
            limpiarCamposComunaDuplicados();
            console.log("Aqui estoy OTRA VEZ CHECKOUT");
        });
    }
    ...
    function processShippingAddress() {
        console.log("Aqui estoy processShippingAddress()");
        ...
        if (shippingAddress !== "") {
            ...
            geocoder.geocode({ address: shippingAddress, componentRestrictions: { country: 'cl' } }, function(results, status) {
                if (status === google.maps.GeocoderStatus.OK && results[0]) {
                    ...
                    // --- NUEVO: Marca el flag y bloquea Región y Comuna ---
                    localStorage.setItem('direccion_buscada', 'true');
                    //lockSelect2Field('#shipping_state');
                    //lockSelect2Field('#shipping_city');
                    // ------------------------------------------------------
                    ...
                    // Si acabas de reemplazar el campo comuna, refuerza:
                    setTimeout(function() {
                        unlockSelect2Field('#shipping_city');
                    }, 100);
                }
    ...
    jQuery(function($){
      ...
      // 1) Función para construir el <select> de comunas según la región
        // Función para construir el <select> de comunas según la región
        function buildCitySelect(regionCode) {
            ...
            var $old = $('#shipping_city').first(); // Solo el primero
            var $new = $('<select>', {
                name: 'shipping_city',
                id:   'shipping_city',
                class: 'city_select',
